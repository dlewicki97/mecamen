    ...
    <script>
      window.addEventListener('load', function () {
        for(let i=1 ; i<3 ; i++) {
          ...
          document.getElementById('photo_logo' + i).addEventListener('change', function () {
            document.getElementById('photoViewer_logo'+ i).innerHTML = '<i class="fas fa-spinner fa-pulse loader"></i>';

            if (this.files && this.files[0]) {
              let src = URL.createObjectURL(this.files[0]);
              setTimeout(function () {
                var photoName = document.getElementById('photo_logo' + i).value;
                document.getElementById('photoViewer_logo' + i).innerHTML = '<img class="img-fluid img-thumbnail" src="' + src + '" width=74>';
                document.getElementById('name_photo_logo' + i).value = photoName.replace(/^.*[\\\/]/, '');
              }, 500);
            }
          });
        }
        document.getElementById('photo_logo' ).addEventListener('change', function () {
            ...
            if (this.files && this.files[0]) {
              ...
              setTimeout(function () {
                ...
                document.getElementById('photoViewer_logo').innerHTML = '<img class="img-fluid img-thumbnail" src="' + src + '" width=74>';
...
function imageIsLoaded(e) { }
  ...
  <script>
    ...
    <script type="text/javascript">
      ...
      function updateFieldPhoto2(field, table) {
      ...
      function updateFieldPhoto3(field, table) {
        var value = document.getElementById(field);
        var data=new FormData();
        data.append(value.name,value.files[0]);
        data.append('table',table);
        $.ajax({  
          type: "post", 
          url:"<?php echo base_url(); ?>panel/settings/photo3", 
          data: data, 
          cache: false,
          contentType: false,
          processData: false,
      ...
      function updateFieldPhoto(field, table) {
        ...
        $.ajax({  
          type: "post", 
          url:"<?php echo base_url(); ?>panel/settings/photo2", 
          data: data, 
          cache: false,
          contentType: false,
          processData: false,
