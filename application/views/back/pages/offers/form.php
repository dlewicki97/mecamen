    ...
    <div class="br-pagebody mg-t-0 pd-x-30">
        ...
        <form class="form-layout form-layout-2" action="<?php echo base_url(); ?>panel/<?php echo $this->uri->segment(2); ?>/action/<?php echo $this->uri->segment(4) . '/' . $this->uri->segment(2); ?>/<?php echo @$offers_category->id. '/'. @$value->id; ?>" method="post"  enctype="multipart/form-data">
                    ...
                    <div class="row">
                        <div class="col-md-12 pr-0">
                            <div class="form-group  bd-t-0-force">
                              <label class="form-control-label mg-b-0-force">Wybierz kategorię: <span class="tx-danger">*</span></label>
                              <select id="select2-a" class="form-control select2-show-search" name="offers_category_id" data-placeholder="Wybierz kategorię" required >
                                <?php foreach($offers_categories as $category): ?>
                                <option value="<?= $category->id ?>" <?php if(@$value->offers_category_id == $category->id) echo 'selected'; elseif(!isset($value))if($category->id == $offers_category->id) echo 'selected' ?>> <?= $category->title ?> </option>
                            <?php endforeach; ?>
                          </select>
                      </div>
                  </div>
