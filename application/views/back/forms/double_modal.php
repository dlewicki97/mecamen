<?php for($i=0 ; $i<2 ; $i++): ?>
  <div class="modal fade" id="modal_<?= $i+1 ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
                        ...
                        <nav aria-label="Page navigation example ">
                          <ul class="pagination modal-pagination pg-elegant justify-content-center mb-0">
                            <?php for($j=0 ; $j< count($api_gallery)/12 ; $j++): ?>
                              <li id="<?= $j+1 ?>" onclick="paginate(<?= $j+1 ?>);setActiveClass(<?= $j+1 ?>, <?= $i+1 ?>)" class="page-item <?php if($j == 0) echo 'active' ?>"><a class="page-link"><?= $j+1 ?></a></li>
                            <?php endfor; ?>
                          </ul>
                        </nav>
                      </div>
    ...
    <script>
      ...
      function setActiveClass(id, i) {
        let modalPagination = document.getElementsByClassName('modal-pagination')[i - 1];
