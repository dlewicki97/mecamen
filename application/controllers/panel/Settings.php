...
class Settings extends CI_Controller {
	...
	public function menu_order() {
		$this->back_m->update('gallery', ['menu_order' => $_POST['menu_order']], $_POST['id']);
	}
	...
	public function photo2() {
		if(checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
			...
			$this->load->library('upload',$config);
			$this->upload->initialize($config);
			$this->upload->do_upload('photo_logo2');
			$data = $this->upload->data();
			if($data['image_width'] > 1440) {
				resizeImg($data['file_name'], $now, '1440');
			} 
			addMedia($data);
			if (!$this->db->field_exists('logo2', 'settings')) $this->base_m->create_column('settings', 'logo2');
			$insert['logo2'] = $now.'/'.$data['file_name'];   
			convert__to__webp($insert['logo2']);
			$this->back_m->update($_POST['table'], $insert, 1);
		} else {
			redirect('panel');
		}
	...
	public function photo3() {
		if(checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
			...
			$this->load->library('upload',$config);
			$this->upload->initialize($config);
			$this->upload->do_upload('photo_logo3');
			$data = $this->upload->data();
			if($data['image_width'] > 1440) {
				resizeImg($data['file_name'], $now, '1440');
			} 
			addMedia($data);
			if (!$this->db->field_exists('logo3', 'settings')) $this->base_m->create_column('settings', 'logo3');
			$insert['logo3'] = $now.'/'.$data['file_name'];   
			convert__to__webp($insert['logo3']);
			$this->back_m->update($_POST['table'], $insert, 1);
		} else {
			redirect('panel');
		}
