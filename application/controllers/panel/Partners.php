...
class Partners extends CI_Controller {
	...
	public function action($type, $table, $id = '') {
		if(checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
			...
			$validTypes = ['image/jpeg', 'image/jpg', 'image/png', 'image/jfif'];
			...
			foreach ($_POST as $key => $value) {
				...
				if($key == 'name_photo_1') {
					if ($this->upload->do_upload('photo_1')) {
						...
						if(in_array($data['file_type'], $validTypes)) convert__to__webp($insert['photo']); 
						addMedia($data);
					...
					if ($this->upload->do_upload('photo_2')) {
						...
						if(in_array($data['file_type'], $validTypes)) convert__to__webp($insert['photo2']); 
						addMedia($data);
					} elseif($value == 'usunięte') {
						$insert['photo2'] = '';
					}
