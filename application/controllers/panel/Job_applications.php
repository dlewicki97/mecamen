...
class Job_applications extends CI_Controller {
	...
	public function send() {
        ...
        $mail->setFrom($cfg['smtp_user'], $data['contact']->company .  ' - stanowisko -'. $_POST['work_place']);
        ...
        $mail->isHTML(true);
        $mail->CharSet = 'UTF-8';
        $mail->Subject = $data['contact']->company .  ' - stanowisko -'. $_POST['work_place'];
        ...
        if(!$mail->send()) {
            ...
            $update['answer'] = 1;
            $this->back_m->update('job_applications', $update, $_POST['id']);  
        }
    }
}