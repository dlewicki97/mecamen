...
class Offers extends CI_Controller {
	...
	public function list($id) {
		if(checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
			...
			$data['rows'] = $this->back_m->get_all_where($this->uri->segment(2), 'offers_category_id', $id);
			$data['offers_category'] = $this->back_m->get_one('offers_categories', $id);
	...
	public function form($type, $offers_category_id, $id = '') {
		if(checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
            // DEFAULT DATA
			$data = loadDefaultData();
            ...
            $data['offers_category'] = $this->back_m->get_one('offers_categories', $offers_category_id);
            $data['offers_categories'] = $this->back_m->get_all('offers_categories');
	...
	public function action($type, $table, $offers_category_id, $id = '') {
		if(checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
			$now = date('Y-m-d');
			...
			redirect('panel/'.$table. '/list/'. $offers_category_id);
		} else {
			redirect('panel');
		}
    }
